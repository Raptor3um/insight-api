...
/**
 * A service for rtmcore to enable HTTP routes to query information about the blockchain.
 *
...
var InsightAPI = function(options) {
  ...
  this.coinTicker = options.coinTicker || 'https://api.coinmarketcap.com/v1/ticker/raptoreum/?convert=USD';
...
InsightAPI.dependencies = ['raptoreumd', 'web'];
...
InsightAPI.prototype.start = function(callback) {
  ...
  this.node.services.raptoreumd.on('tx', this.transactionEventHandler.bind(this));
  this.node.services.raptoreumd.on('block', this.blockEventHandler.bind(this));
...
InsightAPI.prototype.getPublishEvents = function() {
  return [
    ...
    },
    {
        ...
        scope: this,
        subscribe: function (emitter) {
            this.subscribe(emitter, 'raptor');
        }.bind(this),
        unsubscribe: function (emitter) {
            this.unsubscribe(emitter, 'raptor');
        },
        extraEvents: ['raptor/tx', 'raptor/block']
    }
  ];
};
...
InsightAPI.prototype.blockEventHandler = function(hashBuffer) {
  ...
  if (self.subscriptions.raptor.length) {
      ...
      this.blocksController.getBlockByHash(hashBuffer.toString('hex'), function (err, block) {
          ...
          async.mapSeries(block.tx, function(txid, next) {
              ...
              // Notify raptor subscribers
              for (var i = 0; i < self.subscriptions.raptor.length; i++) {
                  self.subscriptions.raptor[i].emit('raptor/block', {
                      block: block,
...
InsightAPI.prototype.transactionEventHandler = function(txBuffer) {
   ...
   if (this.subscriptions.inv.length || this.subscriptions.raptor.length) {
       ...
       if (this.subscriptions.raptor.length) {
           var transformedTrx = this.txController.transformRavenTransaction(tx);

           for (var i = 0; i < this.subscriptions.raptor.length; i++) {
               this.subscriptions.raptor[i].emit('raptor/tx', transformedTrx);
           }
